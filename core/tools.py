...
# ── desktop safe ops (v0.70) – thin wrappers over core.desktop ────

def list_directory(path: str | Path) -> Dict[str, Any]:
    from .desktop import list_directory as _ld, Workspace
    return _ld(path, workspace=None)


def copy_file(src: str | Path, dest: str | Path) -> Dict[str, Any]:
    from .desktop import copy_file as _cf
    return _cf(src, dest, workspace=None, allow_outside=True)


def move_file(src: str | Path, dest: str | Path) -> Dict[str, Any]:
    from .desktop import move_file as _mf
    return _mf(src, dest, workspace=None, allow_outside=True)


def rename_file(path: str | Path, new_name: str) -> Dict[str, Any]:
    from .desktop import rename_file as _rf
    return _rf(path, new_name, workspace=None)


def delete_file(path: str | Path) -> Dict[str, Any]:
    from .desktop import delete_file as _df
    return _df(path, workspace=None, use_trash=True)


def create_folder(path: str | Path) -> Dict[str, Any]:
    from .desktop import create_folder as _cf
    return _cf(path, workspace=None)


def get_system_info() -> Dict[str, Any]:
    from .desktop import get_system_info as _gsi
    return _gsi()


def take_screenshot(dest: str | Path | None = None) -> Dict[str, Any]:
    from .desktop import take_screenshot as _ts
    return _ts(dest, workspace=None)
